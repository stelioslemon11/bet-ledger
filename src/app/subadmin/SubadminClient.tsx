...
export default function SubadminClient({ session, initialPlayers }: { session: Session; initialPlayers: Player[] }) {
  ...
  useEffect(() => {
    if (showAddBet && matchTab === 'browse' && matches.length === 0) {
      fetchMatches()
    }
  ...
  const filteredMatches = matches.filter(m => {
    ...
    const textMatch = !q || m.home.toLowerCase().includes(q) || m.away.toLowerCase().includes(q) || m.league.toLowerCase().includes(q) || m.country.toLowerCase().includes(q)
          ...
          return (
            <div key={player.id} className="rounded-xl p-5" style={{ background: 'var(--surface)', border: '1px solid var(--surface2)' }}>
              ...
              <div className="flex gap-2">
                <button onClick={() => { setBetTargetId(player.id); setShowAddBet(true); setMatchTab('browse'); setMatchSearch('') }} className="flex-1 py-2 rounded-lg text-sm font-medium text-white" style={{ background: 'var(--accent)' }}>+ Add Bet</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.8)' }}>
          <div className="w-full rounded-2xl flex flex-col" style={{ background: 'var(--surface)', border: '1px solid var(--surface2)', maxWidth: matchTab === 'browse' ? '900px' : '480px', maxHeight: '90vh' }}>
            ...
            <div className="flex items-center justify-between p-5 border-b" style={{ borderColor: 'var(--surface2)' }}>
              ...
              <div className="flex gap-2">
                <button onClick={() => setMatchTab('browse')} className="px-4 py-1.5 rounded-lg text-sm font-medium transition-all"
                  style={{ background: matchTab === 'browse' ? 'var(--accent)' : 'var(--surface2)', color: matchTab === 'browse' ? 'white' : 'var(--muted)' }}>
                  🔍 Browse Matches
                </button>
                <button onClick={() => setMatchTab('manual')} className="px-4 py-1.5 rounded-lg text-sm font-medium transition-all"
                  style={{ background: matchTab === 'manual' ? 'var(--accent)' : 'var(--surface2)', color: matchTab === 'manual' ? 'white' : 'var(--muted)' }}>
                  ✏️ Manual
                </button>
              </div>
            </div>
                  ...
                  <div className="flex gap-2">
                    {(['today', 'tomorrow'] as const).map(d => (
                      <button key={d} onClick={() => setSelectedDate(d)} className="px-3 py-2 rounded-lg text-sm font-medium capitalize"
                        style={{ background: selectedDate === d ? '#6366f1' : 'var(--surface2)', color: selectedDate === d ? 'white' : 'var(--muted)' }}>
                        {d}
                      </button>
                    ))}
                  </div>
                ...
                <div className="overflow-y-auto flex-1 p-3">
                  ...
                  {matchError && (
                    <div className="text-center py-10">
                      ...
                      <p className="text-sm mb-3" style={{ color: '#ef4444' }}>{matchError}</p>
                      <button onClick={fetchMatches} className="px-4 py-2 rounded-lg text-sm font-medium text-white" style={{ background: 'var(--accent)' }}>Retry</button>
                    </div>
                  )}
                  {!matchLoading && !matchError && Object.keys(grouped).length === 0 && (
                    <div className="text-center py-10" style={{ color: 'var(--muted)' }}>
                      <div className="text-3xl mb-2">📅</div>
                      <p className="text-sm">No matches found for {selectedDate}{matchSearch ? ` matching "${matchSearch}"` : ''}</p>
                    </div>
                  )}
                  {!matchLoading && !matchError && Object.entries(grouped).map(([leagueKey, leagueMatches]) => (
                    <div key={leagueKey} className="mb-4">
                      <div className="text-xs font-semibold uppercase tracking-wide px-2 py-1.5 mb-1 rounded" style={{ color: '#6366f1', background: '#6366f115' }}>
                        {leagueKey}
                      </div>
                      <div className="space-y-1">
                        {leagueMatches.map(m => (
                          <button key={m.id} onClick={() => selectMatch(m)}
                            className="w-full text-left px-3 py-2.5 rounded-lg transition-all hover:opacity-90"
            ...
            <div className="flex gap-3 p-5 border-t" style={{ borderColor: 'var(--surface2)' }}>
              ...
              {matchTab === 'manual' && (
                <button onClick={addBet} disabled={loading || !betForm.match || !betForm.amount || !betForm.odds}
                  className="flex-1 py-2.5 rounded-lg font-medium text-white disabled:opacity-40"
              ...
              {matchTab === 'browse' && (
                <button onClick={() => setMatchTab('manual')} className="flex-1 py-2.5 rounded-lg font-medium text-white" style={{ background: 'var(--accent)' }}>
                  {betForm.match ? `Continue: ${betForm.match.substring(0, 30)}...` : 'Enter Details →'}
                </button>
              )}
            </div>
          </div>
